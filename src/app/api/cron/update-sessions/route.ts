...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const player of flagged) {
      ...
      await write_Logging({ lg_functionname: 'GET', lg_caller: 'cron/update-sessions', lg_msg: `Phase A: ${player.pl_name} — ${runIds.length} found, ${missing.length} new`, lg_severity: 'I' })
    }
    ...
    if (allMissingIds.size > 0) {
      ...
      for (const run_id of allMissingIds) {
        run_id_count++
        await write_Logging({ lg_functionname: 'GET', lg_caller: 'cron/update-sessions', lg_msg: `Phase C: run_id ${run_id} (${run_id_count}/${allMissingIds.size})`, lg_severity: 'I' })
