//==============================================================================================
//  1) DESCRIPTION
//    POST — /api/scrape/raw/nzb-by-date route handler. Streams (SSE) a raw club-by-date scrape:
//    truncates ts2_results, then for each day fetches the club results page, extracts run_ids,
//    and for the ones missing from tse_sessions does a full parse — upserting a ts1_sessions
//    header and inserting every pair into ts2_results (creating players as needed).
//
//    Parameters:
//      request — JSON body { date_from, date_end, club_id? (default BRIDGE_CLUB_ID) }
//
//    Returns:
//      a text/event-stream Response; per-day progress frames then a final { done: true, … }
//      with run_ids_found / _skipped / _inserted, pairs_inserted, players_created, skipped_rows
//==============================================================================================
    ...
    async start(controller) {
      ...
      try {
        ...
        for (const day of dates) {
          ...
          if (!existingResult.ok) throw new Error('scrape/nzb-by-date/check: ' + existingResult.error)
...
//----------------------------------------------------------------------------------
//  extractRunIds — fast pass: every distinct run_id linked from an event href on
//  the page
//----------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------
//  parsePage — full parse (only run for days with missing sessions): parses the
//  results page into ParsedRow[] grouped by run_id, keeping only rows whose event
//  link carries a run_id and whose score parses
//----------------------------------------------------------------------------------
...
//  VP to VP_SCORE_SANITY_RESET; passes summary-row values and everything else through
//----------------------------------------------------------------------------------
